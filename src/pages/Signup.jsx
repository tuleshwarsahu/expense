  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.session) {
        ...
        setSuccessMessage(
          `Account registered! We have sent a confirmation email to ${email}. Please check your inbox and click the verification link to log in.`
        );
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-slate-50 dark:bg-slate-950 transition-colors">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
        ...
        {successMessage ? (
          <div className="p-5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-900 dark:text-emerald-100 border border-emerald-200 dark:border-emerald-900 text-center space-y-3">
            ...
            <Link
              to="/login"
              className="inline-block px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold shadow-md shadow-emerald-600/20"
            >
              Go to Sign In
            </Link>
          </div>
